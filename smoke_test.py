...
Тестирует:
  ...
  2. Forward pass pipeline для всех temporal_type (cfc, lstm, gru, none, transformer)
  ...
  10. load_backbone_from_checkpoint (backbone init parity)
"""
...
check("2e. Forward [transformer]", lambda: test_forward('transformer'))
...
from models.temporal import CfCTemporalModule, LSTMTemporalModule, GRUTemporalModule, TransformerTemporalModule
...
check("5d. TransformerTemporalModule isolated", lambda: test_temporal_module(TransformerTemporalModule, 'Transformer'))
...
# ──────────────────────────────────────────────
# 10. load_backbone_from_checkpoint — backbone init parity (ТЗ.md, п.11.4)
# ──────────────────────────────────────────────

def test_load_backbone_from_checkpoint():
    import tempfile
    from train import load_backbone_from_checkpoint

    # Источник: модель с temporal_type='cfc' — сохраняем её чекпоинт
    src_model = RetinexLNNPipeline(
        temporal_type='cfc', n_feat=N_FEAT,
        window_size=T, hidden_dim=64, n_neurons=32,
    ).to(DEVICE)
    ...
    # Цель: другая архитектура temporal-модуля ('transformer') — backbone
    # должен быть РАЗНЫЙ до загрузки (разная случайная инициализация)
    dst_model = RetinexLNNPipeline(
        temporal_type='transformer', n_feat=N_FEAT,
        window_size=T, hidden_dim=64, n_neurons=32,
    ).to(DEVICE)

    # До загрузки backbone должен отличаться (разный random seed)
    diff_before = (
        src_model.estimator.conv1.weight - dst_model.estimator.conv1.weight
    ).abs().max().item()
    assert diff_before > 0, "Backbone должен отличаться ДО загрузки (разная инициализация)"

    # Снимок temporal-весов ДО загрузки backbone — должны остаться нетронутыми
    temporal_before = {k: v.clone() for k, v in dst_model.temporal.state_dict().items()}

    load_backbone_from_checkpoint(dst_model, ckpt_path, DEVICE)

    # ПОСЛЕ загрузки backbone должен совпадать ТОЧНО
    diff_after = (
        src_model.estimator.conv1.weight - dst_model.estimator.conv1.weight
    ).abs().max().item()
    assert diff_after == 0.0, f"Backbone должен совпадать ТОЧНО после загрузки, diff={diff_after}"

    diff_denoiser = (
        src_model.denoiser.state_dict()['embedding.weight']
        - dst_model.denoiser.state_dict()['embedding.weight']
    ).abs().max().item()
    assert diff_denoiser == 0.0, f"denoiser тоже должен совпадать точно, diff={diff_denoiser}"

    # Temporal-модуль (Transformer) НЕ должен быть тронут загрузкой backbone
    for k, v_before in temporal_before.items():
        v_after = dst_model.temporal.state_dict()[k]
        assert torch.equal(v_before, v_after), \
            f"Temporal-модуль не должен меняться при load_backbone_from_checkpoint, но {k} изменился"

    os.remove(ckpt_path)
    ok("backbone совпадает точно после загрузки, temporal-модуль (Transformer) не тронут")

check("10. load_backbone_from_checkpoint (backbone init parity)", test_load_backbone_from_checkpoint)
