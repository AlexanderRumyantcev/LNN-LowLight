...
Для сцен с крупными кадрами (например, 1080p) полная упаковка может требовать
держать на диске ОДНОВРЕМЕННО и существующий .npy-кэш, и новый .blob — то есть
почти 2x места сцены, что на Kaggle (лимит /kaggle/working обычно ~20GB) может
не влезать уже для одной сцены 1080p (~13.6GB .npy + ~13.6GB .blob = ~27GB).
Для проверки гипотезы достаточно подмножества кадров — используй
--max-frames-per-level N, чтобы ограничить упаковку первыми N кадрами КАЖДОГО
level (low_light_10/20, normal_light_10/20), а не общим числом кадров —
это сохраняет одинаковое число кадров на каждый из 4 levels, что важно для
BlobSceneDataset (он строит окна window_size из пар low/gt одного level, и
неравное число кадров между levels исказило бы число получаемых сэмплов):

    python scripts/pack_scene_to_blob.py \
        --npy-cache-root /kaggle/working/bvi_rlv_npy \
        --data-root /kaggle/input/bvi-rlv-low-light-video-dataset-20-scene-subset \
        --scene S02 \
        --max-frames-per-level 250 \
        --out /kaggle/working/bvi_rlv_blob \
        --verify
...
не оставляя битых артефактов). Также проверено --max-frames-per-level: корректно
обрезает каждый level независимо (не общий список), сохраняя кадры с НАЧАЛА
последовательности (детерминированно — не случайная выборка), и работает
вместе с --verify на обрезанном поднаборе.
"""
...
def collect_frame_paths(content_dir: Path, npy_cache_root: Path, data_root: Path, max_frames_per_level=None):
    """
    ...
    max_frames_per_level: если задан, каждый level независимо обрезается до
    первых N кадров (в сортированном порядке — с начала последовательности,
    не случайная выборка) ДО формирования pairs. Делается отдельно на каждый
    level (а не на общий список всех 4 levels), чтобы число кадров в
    low_light_10 совпадало с числом в normal_light_10 и т.д. — иначе
    BlobSceneDataset._frames_by_level получит несимметричные level'ы и число
    окон (window_size-сэмплов) исказится по сравнению с полной сценой.
    ...
    for level in ALL_LEVELS:
        ...
        if max_frames_per_level is not None:
            png_paths = png_paths[:max_frames_per_level]
        pairs = []
...
def main():
    ...
    parser.add_argument(
        '--max-frames-per-level', type=int, default=None,
        help='Опционально: ограничить упаковку первыми N кадрами КАЖДОГО level '
             '(low_light_10/20, normal_light_10/20) — для проверки гипотезы на '
             'подмножестве, когда на диске не хватает места для полной сцены '
             '(нужно ~2x места: существующий .npy-кэш + новый .blob).',
    )
    parser.add_argument(
    ...
    if args.max_frames_per_level is not None and args.max_frames_per_level <= 0:
        print(f"[ERROR] --max-frames-per-level должен быть > 0, получено {args.max_frames_per_level}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.max_frames_per_level is not None:
        print(f"      Ограничение: первые {args.max_frames_per_level} кадров на каждый level")

    frames_by_level = collect_frame_paths(
        content_dir, npy_cache_root, data_root,
        max_frames_per_level=args.max_frames_per_level,
    )
