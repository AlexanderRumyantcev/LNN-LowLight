...
def build_all_pixel_sequences(
    ...
    """Список dict — одна запись на каждый (валидный хотя бы один раз за
    ...
    for y in range(H):
        for x in range(W):
            if not valid[:, y, x].any():
                continue  # пиксель ни раз не попал в геометрию сцены
            ...
            # Позиционное кондиционирование (2026-08-20, models/spatial_features.py, чат):
            # position/normal/albedo уже отрендерены per-frame per-pixel в
            # generate_dataset_dense.py (тот же рендер, что даёт irradiance/spp) — здесь
            # просто извлекаются по (y, x), без доп. рендера. direction — МОЯ инженерная
            # реализация (не отдельный буфер): normalize(position - camera_pos), тот же
            # принцип, что у настоящего NRC (view direction относительно камеры, не
            # относительно нормали/сцены). Единственное отличие от probe-пути: здесь
            # position/normal/albedo ВРЕМЕННО меняются в пределах одной последовательности
            # (экранный пиксель — не world-space точка, см. докстринг модуля выше про
            # структурное отличие), т.е. модель видит их как часть temporal input, не как
            # статичный контекст.
            position_seq = position[:, y, x, :].astype(np.float32)
            normal_seq = normal[:, y, x, :].astype(np.float32)
            albedo_seq = albedo[:, y, x, :].astype(np.float32)
            diff = position_seq - camera_positions.astype(np.float32)
            direction_seq = diff / (np.linalg.norm(diff, axis=-1, keepdims=True) + 1e-8)
